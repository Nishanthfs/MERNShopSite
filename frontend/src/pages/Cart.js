...
const fetchData =async()=>{
    ...
    const responce = await fetch(summeryapi.addToCartProductView.url,{
...
const handleLoading =async()=>{
 ...
 const increaseQty = async(id,qty)=>{
  const responce = await fetch(summeryapi.updateCartProduct.url,{
...
const deleteCartProduct =async(id)=>{
  const responce = await fetch(summeryapi.deleteCartproduct.url,{
    ...
    <div className='container mx-auto'>
      ...
      <div className='text-center text-lg my-3'>  
         {
             data.length === 0 && !loading && (
               <p className='bg-white py-5'>NO</p>
             )
         }
     </div>
     ...
     <div className='flex flex-col lg:flex-row gap-10 justify-between p-4'>
          ...
          <div className='w-full max-w-3xl'>
            {
                loading ?(
                    loadingCart.map((el,index)=>{
                        return(
                       <div key={el+"Add to Cart Loading"+index} className='w-full bg-slate-200 h-32 my-2 border border-slate-200 animate-pulse rounded'>
                        ...
                        <div key={product?._id+"Add to Cart Loading"} className='w-full bg-white h-32 my-2 border border-slate-300 rounded grid grid-cols-[128px,1fr] '>
                         ...
                         <div className='px-4 py-2 relative'>
                          ...
                          <MdDelete />
                            ...
                            <p className='captalize text-slate-500'>{product?.productId.category}</p>
          ...
          <div className='mt-5 lg:mt-0 w-full max-w-sm'>
           {
           loading ? (
               <div className='h-28 bg-slate-200 border border-slate-300 animate-pulse'>
             ...
             <div className='h-36 bg-white'>
               ...
               <button className='bg-blue-600 p-2 text-white w-full mt-5'>Payment</button>
